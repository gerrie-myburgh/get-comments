...
fn main() {
    if let Ok(cli) = parse_command_line() {
        ...
        if some_dir.is_some() && some_work.is_some() && some_start.is_some() && some_path.is_some()
        {
            ...
            println!(
                "Missing command line -dir source_folder -work document_root -start comment_start -path legal_folder_prefix"
            )
        }
    }
}
